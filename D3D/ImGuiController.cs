...
namespace ImGuiNET
{
    ...
    public class ImGuiController : IDisposable
    {
        ...
        public ImGuiController(GraphicsDevice gd, OutputDescription outputDescription, int width, int height)
        {
            ...
            ImFontPtr font = io.Fonts.AddFontFromFileTTF("C:\\Windows\\Fonts\\simsun.ttc", 18.0f, null, io.Fonts.GetGlyphRangesChineseFull());
        ...
        private byte[] LoadEmbeddedShaderCode(ResourceFactory factory, string name, ShaderStages stage)
        {
            switch (factory.BackendType)
            {
                case GraphicsBackend.Direct3D11:
                {
                    ...
                    string resourceName = projectName +".D3D.Shaders.HLSL." + name + ".hlsl.bytes";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                case GraphicsBackend.OpenGL:
                {
                    string projectName = Assembly.GetExecutingAssembly().GetName().Name.ToString();
                    string resourceName = projectName + ".D3D.Shaders.GLSL." + name + ".glsl";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                case GraphicsBackend.Vulkan:
                {
                    string projectName = Assembly.GetExecutingAssembly().GetName().Name.ToString();
                    string resourceName = projectName + ".D3D.Shaders.SPIR-V." + name + ".spv";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                case GraphicsBackend.Metal:
                {
                    string projectName = Assembly.GetExecutingAssembly().GetName().Name.ToString();
                    string resourceName = projectName + ".D3D.Shaders.Metal." + name + ".metallib";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                default:
