...
namespace WPFCheatUITemplate.GameMode
{
    ...
    class GameModeManger
    {
        ...
        public static List<Zombie> GetZombies()
        {
            ...
            int maxnum = CheatTools.ReadMemoryPoninter<int>(AppGameFunManger.Instance.Handle, zombieMaxNumOffset.pointer);


            for (int i = -maxnum; i < maxnum; i++)
            {
                int address = CheatTools.ReadMemoryPoninter<int>(AppGameFunManger.Instance.Handle, 
                ...
                if (address == 0)
                {
                    int BaseAddress = CheatTools.ReadMemoryPoninter<int>(AppGameFunManger.Instance.Handle, zombieOffset.pointer) + zombieOffset.sizeOrOffset * i;
